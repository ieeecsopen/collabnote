...
import { Sparkles, Loader, Share2, Clock, MoreHorizontal, MessageSquare, Star } from 'lucide-react';
  ...
  const handleAISubmit = async (e: React.FormEvent) => {
    ...
    if (lines.length > 0) {
        const newBlocks: Block[] = lines.map(line => ({
            id: crypto.randomUUID(),
            type: line.startsWith('- ') ? 'bullet-list' : line.startsWith('# ') ? 'heading-1' : 'paragraph',
        ...
        if (currentBlock.content.trim() === '') {
            currentBlocks.splice(blockIndex, 1, ...newBlocks);
        } else {
             currentBlocks.splice(blockIndex + 1, 0, ...newBlocks);
        }
    ...
    <div className="flex flex-col h-full bg-white relative overflow-hidden">
      ...
      <header className="flex-none bg-white border-b border-slate-100 z-30 px-6 py-3 flex items-center justify-between">
         ...
         <div className="flex items-center gap-2">
             ...
             <button className="ml-2 h-8 inline-flex items-center justify-center rounded-lg text-sm font-medium transition-colors bg-slate-900 text-white shadow-lg shadow-slate-900/20 hover:bg-slate-800 px-4">
                 Share
             </button>
         </div>
      </header>
