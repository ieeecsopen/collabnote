...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const navItemClass = (isActive: boolean) => 
    `w-full flex items-center gap-2 px-3 py-1.5 text-sm font-medium rounded-lg transition-all duration-200 ${
        isActive 
    ...
    <div className="w-[260px] h-full flex flex-col text-slate-300">
      ...
      <div className="mb-4 pt-1">
        <button className="w-full flex items-center gap-3 px-3 py-2 hover:bg-slate-800/50 rounded-lg transition-colors text-left group">
            <div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg text-white flex items-center justify-center font-bold text-xs shadow-lg shadow-indigo-500/20">
                C
            </div>
                ...
                <div key={workspace.id} className="mb-2">
                    {workspace.folders.map(folder => (
                        <div key={folder.id} className="mb-1">
                            <div 
                                className={`group flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg cursor-pointer select-none transition-colors ${
                                    false ? 'text-white' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/30'
                                }`}
                                    ...
                                    {folder.documents.map(doc => (
                                        <div 
                                            ...
                                            className={`group flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg cursor-pointer transition-all ${
                                                activeDocId === doc.id && currentView === 'editor'
